...
def evaluar_fitness(individuo, puertos, consumidores):
...
def busqueda_tabu(individuo_inicial, puertos, consumidores):
    ...
    mejor_fitness_global = evaluar_fitness(mejor_solucion, puertos, consumidores)
    ...
    for _ in range(TS_ITERACIONES):
        ...
        for _ in range(20):
            ...
            fitness_vecino = evaluar_fitness(vecino, puertos, consumidores)
...
if __name__ == "__main__":
    ...
    if producer_df is not None:
        ...
        print("\n--- INICIANDO ALGORITMO HÍBRIDO (HGA-TS) CON CLUSTERS PRE-CALCULADOS ---")
        ...
        poblacion = [crear_individuo_inteligente(producer_df, consumer_df, params['N_BARCOS'], params['CAPACIDAD_BARCO']) for _ in range(TAMANO_POBLACION)]
        for ind in poblacion:
            evaluar_fitness(ind, puertos_df, consumers_list)
        ...
        for gen in range(N_GENERACIONES):
            ...
            mejor_refinado = busqueda_tabu(mejor_de_generacion, puertos_df, consumers_list)
            ...
            fitness_promedio = np.mean([ind['fitness'] for ind in poblacion if ind['fitness'] > -float('inf')])
            
            print(f"Generación {gen+1:2d}: Mejor Fitness = {mejor_fitness_actual:10.2f} (Mejor Global: {mejor_fitness_global:10.2f})")

        print("\n--- EVOLUCIÓN COMPLETADA ---")
        print(f"Mejor fitness global encontrado: {mejor_fitness_global:.2f}")


        mejor_individuo_final = max(poblacion, key=lambda x: x['fitness'])
        print(f"Mejor fitness global encontrado: {mejor_individuo_final['fitness']:.2f}")

        
        # --- NUEVO: ANÁLISIS DE LA MEJOR SOLUCIÓN ENCONTRADA ---
        print("\n" + "="*50)
        print("--- ANÁLISIS DEL MEJOR PLAN SEMANAL ENCONTRADO ---")
        print("="*50)

        # Ordenar las rutas por número de paradas para ver las más complejas primero
        rutas_ordenadas = sorted(mejor_individuo_final['rutas'], key=lambda r: len(r['ruta']), reverse=True)

        total_entregado = 0
        consumidores_atendidos = set()

        for plan_buque in rutas_ordenadas:
            ...
            productor_id = ruta[0]['puerto_id']
            paradas_descarga = [p['puerto_id'] for p in ruta[1:]]
            
            print(f"\n- Buque {buque_id}:")
            print(f"  - Carga en: {productor_id}")
            print(f"  - Número de entregas: {len(paradas_descarga)}")
            print(f"  - Secuencia de visitas: {' -> '.join(paradas_descarga)}")
        ...
        print("\n" + "="*50)
        print("--- RESUMEN GENERAL DEL PLAN ---")
        print(f"  - Fitness (Utilidad): {mejor_individuo_final['fitness']:.2f}")
        print(f"  - Unidades Totales Entregadas: {total_entregado}")
