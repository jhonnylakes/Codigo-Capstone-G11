# Archivo: test_heuristica.py (Versión con Lógica de Inventario Correcta)
...
def crear_individuo_economico(productores_df, consumidores_df, n_barcos, capacidad, puertos_df, estado_inventarios):
    ...
    # MODIFICACIÓN: La lista se basa en la demanda PENDIENTE real.
    ...
    for i in range(1, int(n_barcos) + 1):
        ...
        while len(ruta_buque['ruta']) < max_paradas and consumidores_pendientes:
            ...
            # MODIFICACIÓN: La cantidad a entregar es la demanda pendiente
...
def evaluar_fitness(individuo, puertos, consumidores, estado_inventarios_inicial, params):
    ...
    for i, plan in enumerate(individuo['rutas']):
        ...
        # Validación de ruta
        carga_actual = 0 # Para una semana sola, el barco siempre empieza vacío antes de la primera carga
        for parada in ruta:
        ...
        # Calcular costos de viaje
        if len(ruta) < 2: continue
        # En standalone, el primer viaje siempre es desde el primer puerto de la ruta
    ...
    # --- LÓGICA DE CÁLCULO ECONÓMICO CORREGIDA ---
    ingreso_total = 0
    dns_total = 0
...
# (Resto de funciones: seleccion, cruzamiento, mutacion, etc.)
...
if __name__ == "__main__":
    ...
    if producer_df is not None:
        ...
        # --- ESTADO INICIAL DUMMY (para pruebas standalone) ---
        ...
        for ind in poblacion:
            # En modo standalone, el estado de los barcos es irrelevante para la evaluación
